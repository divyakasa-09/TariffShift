...
# Main execution
def main():
    # United States (840) as reporter, India (356) as partner
    reporter_code = "840"
    partner_code = "356"
    year = "2019"
    ...
    if not tariff_df.empty:
        tariff_df.to_csv("us_mfn_tariffs_india_2019.csv", index=False)
        print("MFN tariff data saved to us_mfn_tariffs_2019.csv")
        print("\nSample tariff data:")
        print(tariff_df.head())
    
    if not import_df.empty:
        import_df.to_csv("us_imports_from_india_2019.csv", index=False)
        print("Import data saved to us_imports_from_india_2019.csv")
        print("\nSample import data:")
    ...
    if not tariff_df.empty and not import_df.empty:
        ...
        if "ProductSector" in tariff_df.columns and "ProductSector" in import_df.columns:
            ...
            combined_df.to_csv("us_tariffs_and_imports_india_2019.csv", index=False)
            print("\nCombined data saved to us_tariffs_and_imports_india_2019.csv")
